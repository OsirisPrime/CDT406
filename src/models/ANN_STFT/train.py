...
def build_and_train_best_model(input_shape, num_classes, best_hp, X_train, y_train, X_val, y_val):
    # Create ANN_STFT model with best hyperparameters
    model = ANN_STFT(
...
def save_best_model(model, pre_processor_variant):
    model_dir = get_models_dir() / "ANN_STFT_search" / "best_ANN_STFT_models"  # Changed directory name
    model_dir.mkdir(parents=True, exist_ok=True)
    model.save(model_dir / f"ANN_STFT_variant_{pre_processor_variant}.keras")  # Changed filename
...
def get_results_and_save_models(folder_path, variant_folders):
    ...
    for variant_num, variant_folder in variant_folders.items():
        ...
        print(f"[INFO] Input shape for ANN_STFT: {input_shape}")  # Added info log
    ...
    plt.suptitle("Confusion Matrices for All Variants (ANN_STFT)", fontsize=14)  # Updated title
    plt.tight_layout()
    plt.subplots_adjust(top=0.85)
    conf_matrix_path = get_models_dir() / "ANN_STFT_search" / "best_ANN_STFT_models" / "confusion_matrix.png"  # Updated path
...
if __name__ == "__main__":
    ANN_search_folder_path = str(get_models_dir()) + "/ANN_STFT_search"  # Changed folder path
    ...
    ANN_results = get_results_and_save_models(ANN_search_folder_path, variant_folders)  # Changed variable name

    print("\nANN_STFT model training and saving complete.")  # Updated message